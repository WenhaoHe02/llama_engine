...
void launchInputEmbedding(TensorWrapper<int>* input_ids,  // INT [token num]
                          ...
                          EmbeddingWeights<T>* embedding_table) { // FP32 [vocal_size, hidden_size] = [65536, 4096]

                            constexpr int block_size = 256;
                            constexpr int grid_size = 2048;
                            const int max_context_token_num = input_ids->shape[0];
                            const int hidden_size = output->shape[1];
                            LLM_CHECK_WITH_INFO(max_context_token_num == output->shape[0], "input ids 1st shape should equal to 1st shape of output");
                            input_embedding_kernel<T><<<grid_size, block_size>>>(input_ids->data, output->data, embedding_table->data, max_context_token_num, hidden_size);
                            #ifdef PRINT_DATA
...
template void launchInputEmbedding(TensorWrapper<int>* input_ids,
    ...
    EmbeddingWeights<float>* embedding_table);
template void launchInputEmbedding(TensorWrapper<int>* input_ids,
    TensorWrapper<half>* output,
    EmbeddingWeights<half>* embedding_table);